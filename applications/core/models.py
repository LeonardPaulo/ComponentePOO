...
class TipoSangre(models.Model):
    ...
    descripcion = models.CharField(
        max_length=100, verbose_name="Descripcion", blank=True, null=True
    )
...
class Paciente(models.Model):
    ...
    fecha_nacimiento = models.DateField(
        verbose_name="Fecha de Nacimiento", help_text="Formato: AAAA-MM-DD"
    )
    ...
    sexo = models.CharField(
        ...
        verbose_name="Sexo",
        default=SexoChoices.MASCULINO,
    )
    estado_civil = models.CharField(
        max_length=12, choices=EstadoCivilChoices.choices, verbose_name="Estado Civil"
    )
...
class Doctor(models.Model):
    ...
    foto = models.ImageField(
        upload_to="core/doctores/", verbose_name="Foto", null=True, blank=True
    )
    ...
    def get_foto_url(self):
        """
        Obtiene la URL de la foto con validación de existencia del archivo.
        Retorna None si el archivo no existe.
        """
        try:
            if self.foto and self.foto.storage.exists(self.foto.name):
                return self.foto.url
            return None
...
class Empleado(models.Model):
    ...
    cargo = models.ForeignKey(
        "Cargo", on_delete=models.PROTECT, verbose_name="Cargo", related_name="cargos"
    )
...
class Medicamento(models.Model):
    ...
    cantidad = models.PositiveIntegerField(
        verbose_name="Stock", help_text="Cantidad actual disponible en inventario."
    )
    ...
    foto = models.ImageField(
        upload_to="core/medicamentos/",
        verbose_name="Foto del Medicamento",
        null=True,
        blank=True,
    )
...
class FotoPaciente(models.Model):
    ...
    def __str__(self):
        return (
            f"Foto de {self.paciente} ({self.fecha_subida.strftime('%Y-%m-%d %H:%M')})"
        )

    def get_image_url(self):
        """
        Obtiene la URL de la imagen con validación de existencia del archivo.
        Retorna None si el archivo no existe.
        """
        try:
    ...
    def get_image_size(self):
        """
        Obtiene el tamaño de la imagen con validación de existencia del archivo.
        Retorna 'N/A' si el archivo no existe.
        """
        try:
            if self.imagen and self.imagen.storage.exists(self.imagen.name):
    ...
    def file_exists(self):
        ...
        """
        try:
            return self.imagen and self.imagen.storage.exists(self.imagen.name)
        except (FileNotFoundError, OSError, ValueError):
            return False
